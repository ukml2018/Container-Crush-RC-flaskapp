...
def find():
    ...
    if request.method == 'POST':
        ...
        query_string = "select pr.item_number itemid, sku.description descp, pr.list_price rprice, pr.price_effective_date edate, (pr.list_price -  IFNULL(pr.discount,0)) dis, sku.SKU_ATTRIBUTE_VALUE1 size, sku.SKU_ATTRIBUTE_VALUE2 color, concat('https://objectstorage.us-phoenix-1.oraclecloud.com/n/ibmoracledt/b/imageContainerCrush/o/',concat(pr.item_number,'.jpg'))  image from XXIBM_PRODUCT_PRICING pr, XXIBM_PRODUCT_SKU sku where pr.item_number = sku.item_number and lower(sku.description) like  lower(%s)";
